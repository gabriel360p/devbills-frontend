...
//puxando algumas informações para tipagem do button do html
...
const Button = ({
    children,
    //por padrão o botão é do tipo primary (caso não seja falado qual a variante, ele define como primary)
    variant = 'primary',
    fullWidth = false,
    isLoading = false,
    //className e disabled já está incluso no ButtonHTMLAttributes
    className,
    disabled,
    ...rest
}: ButtonProps) => {
    ...
    return (
        ...
        px-5 py-2.5 rounded-xl font-medium cursor-pointer transition-all flex items-center 
        justify-center ${variantClasses[variant]}
        ${className}
